...
export class InvalidateCacheInterceptor implements NestInterceptor {
  ...
  private async invalidateCache(options: InvalidateCacheOptions, request: any): Promise<void> {
    try {
      ...
      if (options.endpoint) {
        ...
        if (options.includeUserId && request.user?.id) {
          ...
          await this.cacheService.invalidateEndpoint(options.module, options.endpoint);
        }
      } else {
