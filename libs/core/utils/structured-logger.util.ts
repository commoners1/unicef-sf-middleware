...
export class StructuredLogger {
   ...
   */
  jobComplete(jobId: string, processingTime: number, context?: Record<string, unknown>): void {
    this.logger.log({
      type: 'job_complete',
      jobId,
      processingTime,
   ...
   * Log batch operation
   */
  batch(operation: string, count: number, duration: number, context?: Record<string, unknown>): void {
    this.logger.log({
   ...
   */
  alert(type: string, message: string, severity: 'WARNING' | 'CRITICAL', metrics?: Record<string, unknown>): void {
