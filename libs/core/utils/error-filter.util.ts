...
import { buildOrderBy, extractPagination, parseColumnFilters, buildColumnFilterWhere, parseBoolean, buildDateRangeFilter } from './dynamic-filter.util';
...
export class ErrorFilterBuilder {
  static buildBaseFilters(
    ...
    if (columnFilters && Object.keys(columnFilters).length > 0) {
      const columnFilterConditions = buildColumnFilterWhere(columnFilters as any);
  ...
  static buildQuery(filters: ErrorLogFilters) {
    ...
    const orderBy = buildOrderBy(filters.sortBy, filters.sortOrder || 'desc', allowedSortFields, 'timestamp');
