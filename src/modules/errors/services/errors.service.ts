...
export class ErrorsService {
  ...
  async findAll(query: ErrorLogFiltersDto | any) {
    ...
    const sanitizedQuery = SanitizationUtil.sanitizeObject(
      query,
    ) as ErrorLogFilters;

    // Sanitize string fields
    if (sanitizedQuery.search) {
      sanitizedQuery.search = SanitizationUtil.sanitizeSearchQuery(
        sanitizedQuery.search,
      );
    }
    if (sanitizedQuery.type) {
      sanitizedQuery.type = SanitizationUtil.sanitizeString(
        sanitizedQuery.type,
      );
    }
    if (sanitizedQuery.source) {
      sanitizedQuery.source = SanitizationUtil.sanitizeString(
        sanitizedQuery.source,
      );
    }
    if (sanitizedQuery.environment) {
      sanitizedQuery.environment = SanitizationUtil.sanitizeString(
        sanitizedQuery.environment,
      );
    }

    // Build query using ErrorFilterBuilder
    const { where, orderBy, skip, take, page, limit } =
      ErrorFilterBuilder.buildQuery(sanitizedQuery);
  ...
  async getStats() {
    ...
    const avg =
      allErrors.length > 0
        ? Math.round(
            allErrors.reduce(
              (a: number, b: any) => a + (b.occurrences || 1),
              0,
            ) / allErrors.length,
          )
        : 0;
  ...
  async resolve(id: string, resolvedBy: string) {
    ...
    return this.prisma.errorLog.update({
      ...
      data: {
        resolved: true,
        resolvedAt: new Date(),
        resolvedBy: sanitizedResolvedBy,
      },
    });
  }
  ...
  private async convertToXlsx(data: any[]): Promise<Buffer> {
    ...
    if (data.length === 0) {
      ...
      worksheet.columns = emptyHeaders.map((header) => ({
        header,
