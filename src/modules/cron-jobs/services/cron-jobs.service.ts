...
export class CronJobsService {
  ...
  async getCronJobs(filters: {
    ...
    const cronJobs: CronJobInfo[] = jobs.map((job: any) => {
      ...
      return {
        ...
        lastStatusMessage: job.responseData
          ? JSON.stringify(job.responseData)
          : null,
  ...
  async runCronJob(jobType: string) {
    ...
    try {
      ...
      if (typeof scheduler[methodName] === 'function') {
        ...
        throw new Error(
          `Method ${methodName} not found in JobSchedulerService`,
        );
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async getJobState(jobType: string): Promise<boolean> {
    return await this.cronJobStateService.getJobState(jobType);
  }

  async getAllJobStates(): Promise<Record<string, boolean>> {
    return await this.cronJobStateService.getAllStates();
  }
