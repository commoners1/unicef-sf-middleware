...
import { GroupByUtil } from '@utils/group-by.util';
import {
  AuditFilterBuilder,
  type AuditLogFilters,
} from '@utils/audit-filter.util';
import {
  buildOrderBy,
  extractPagination,
} from '@utils/dynamic-filter.util';
import { SALESFORCE_METHODS, CRON_JOB_METHODS } from '@utils/constants';
...
export class AuditService {
    ...
    return this.prisma.auditLog.create({
      data: {
        ...
        requestData: requestData
          ? (requestData as Prisma.InputJsonValue)
          : undefined,
  ...
  async getUndeliveredCronJobs(
    ...
    const sanitizedUserId = userId
      ...
      : null;
    const sanitizedJobType = jobType
      ? SanitizationUtil.sanitizeString(jobType)
      : undefined;
    const sanitizedMaxLimit = maxLimit
      ? Math.min(Math.max(Number(maxLimit), 1), 10000)
      : undefined;
    ...
    return this.prisma.auditLog.findMany({
      ...
      ...(sanitizedMaxLimit &&
        sanitizedMaxLimit > 0 && { take: sanitizedMaxLimit }),
      include: {
        apiKey: {
          select: { name: true, description: true },
        },
  ...
  private async getLogsWithFilters(
    ...
    if (sanitizedFilters.action) {
      sanitizedFilters.action = SanitizationUtil.sanitizeString(
        sanitizedFilters.action,
      );
    }
    if (sanitizedFilters.method) {
      sanitizedFilters.method = SanitizationUtil.sanitizeString(
        sanitizedFilters.method,
      );
    }
    if (sanitizedFilters.userId) {
      sanitizedFilters.userId = SanitizationUtil.sanitizeString(
        sanitizedFilters.userId,
      );
    }
    if (sanitizedFilters.apiKeyId) {
      sanitizedFilters.apiKeyId = SanitizationUtil.sanitizeString(
        sanitizedFilters.apiKeyId,
      );
    }
  ...
  async getTopEndpoints() {
    ...
    const totalRequests = endpointStats.reduce(
      (sum: number, stat: any) => sum + stat._count.endpoint,
      0,
    );
    ...
    return endpointStats.map((stat: any) => ({
      ...
      percentage:
        Math.round((stat._count.endpoint / totalRequests) * 100 * 10) / 10,
    }));
  }
