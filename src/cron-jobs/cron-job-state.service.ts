  ...
  private async loadStatesFromDatabase() {
    try {
      ...
      for (const setting of settings) {
        const enabled = setting.valueType === 'boolean' 
          ? setting.value === 'true' 
          : true; // Default to enabled if parsing fails
        this.jobStates.set(setting.key, enabled);
      }

      this.logger.log(`Loaded ${settings.length} cron job states from database`);
    } catch (error) {
  ...
  private async initializeDefaultSettings() {
    ...
    for (const { key, enabled } of defaultStates) {
      ...
      try {
        ...
        this.logger.error(`Failed to create default setting for ${key}:`, error);
      }
    }
